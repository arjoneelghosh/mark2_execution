  ...
  siteMeta: {
    ...
    chatbotBehavior: {
      ...
      bullets: [
        ...
        "Answers are generated by a Groq-hosted Llama model wrapper when online, with this local keyword engine as the fallback."
      ],
    ...
    chatbotPowering: {
      ...
      summary: "The Ask assistant is a two-tier system: a lightweight wrapper around a Groq-hosted open-weight Llama model, with this local grounded engine as its offline fallback.",
      bullets: [
        "Primary path: questions go to a serverless endpoint (/api/chat) that calls the Groq API (llama-3.1-8b-instant, an open-weight Llama model) — the assistant is a transparent LLM wrapper, not a custom-trained model.",
        "The LLM is grounded in the site's machine-readable llms.txt summary, generated at build time from the same data the pages render, with strict instructions to answer only from that content.",
        "Fallback path: if the LLM is unavailable, rate-limited, or the site runs offline (like local development), this fully client-side engine answers instead using local intent routing and bundled portfolio retrieval.",
        "Both paths refuse unsupported questions instead of guessing."
      ],
